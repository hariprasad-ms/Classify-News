...
`This repository contains the my research project reguarding News Category Prediction as an aspect for the new feature development of Destiny, as well as for Personal Knowledge.`

---

This project reads a dataset of news articles from a CSV file using Pandas and processes the data using Scikit-learn's CountVectorizer to convert the text data into a matrix of token counts. The processed data is then split into training and testing sets using Scikit-learn's train_test_split method. A Multinomial Naive Bayes model is trained on the training data using Scikit-learn's MultinomialNB module.

The user is then prompted to input a text string, which is processed using the CountVectorizer object and used to predict the category of the input text using the trained model. The predicted category is then printed to the console.

Overall, this project demonstrates the use of Scikit-learn's machine learning modules to process and analyze text data, and provides a simple implementation of a text classification model.
